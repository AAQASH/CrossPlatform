$(document).ready(function(){
	...
	$( "#date" ).datepicker({ dateFormat: 'yy-mm-dd', maxDate: "+18Y", changeMonth: true, changeYear: true });
	$('#time').timepicker();
});
...
/*jQuery.validator.addMethod("checkDuplicateEmail", function(value, element, params) {
    var email = $("#email").val()
    var param = $.extend({ "type": "validate_email", "edit_id": $("#edit_id").val(), "email" : email, "action": "edit" }, param);
	var is_valid = false;
    return eval($.ajax({
        type: "POST",
        url: "ajax/user.php",
...
}, jQuery.validator.format("Email id is already in use"));*/
