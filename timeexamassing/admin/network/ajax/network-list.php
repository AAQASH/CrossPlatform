...
include_once("../../bootstrap.php");
	...
	try{
		...
		$grid->select($select)->from('mts_user')->where("account_type_id_fk=2")->orderby($order_by)->limit();
		$row = $grid->query();
		$grid->SQL;
...
<div>
  <form name="grid" id="grid">
    <table width="100%" border="0" cellpadding="0" cellspacing="0" id="my_table">
      <tr>
        <td><div class="page-header">
            <div class="fl">
              <h1><small>Network Users List</small></h1>
            </div>
            ...
            <div class="fr">
              ...
              &nbsp;
              <input type="button" name="btnaddnew" id="btnaddnew" value=" Add New User " class="btn btn-warning btn-sm" onclick="javascript:location.href='network-save.php';" />
            </div>
            <div class="clr"></div>
            </div>
          	<div class="clr"></div>
    ...
    <table width="100%" border="0" id="my_table" class="tblformat">
      ...
      <thead>
        <tr>
          <th width="2%" style="cursor:pointer;">&nbsp;</th>
          <th width="2%" style="cursor:pointer;" abbr="UID"><div>ID</div></th>
          <th width="6%" style="cursor:pointer;" abbr="roll_no">Roll No</th>
          <th width="15%" style="cursor:pointer;" abbr="email"><div>Email</div></th>
          <th width="15%" style="cursor:pointer;" abbr="firstname"><div>First Name</div></th>
          <th width="15%" style="cursor:pointer;" abbr="lastname"><div>Last Name</div></th>
          <th width="10%" style="cursor:pointer;" abbr="is_active"><div>Active</div></th>
          <th width="9%" style="cursor:pointer;" abbr="birthdate"><div>Birth Date</div></th>
          <th width="8%" style="cursor:pointer;" abbr="signupdate"><div>Signup Date</div></th>
          <th width="6%" style="cursor:pointer;" abbr="is_verified"><div>Verified</div></th>
          <th width="6%" style="cursor:pointer;" abbr="is_blocked"><div>Block</div></th>
          <th width="6%" style="cursor:pointer;" abbr=""><div>Action</div></th>
        </tr>
        <tr class="search">
          ...
          <td><input class="form-control" type="text" name="roll_no" id="roll_no" value="<?php echo stripslashes($_REQUEST['roll_no']); ?>" size="3" /></td>
	...
	if(sizeof($row) > 0){
        ...
        <tr>
          ...
          <a href="<?php echo SITEROOTADMIN?>/network/network-save.php?edit_id=<?php echo $row[$i]["UID"]?>" title="Edit"><span class="glyphicon glyphicon-pencil"></span></a>&nbsp;<a href="javascript:void(0);" class="delete" id="<?php echo $row[$i]["UID"]?>"><span class="glyphicon glyphicon-remove"></span></a></td>
        </tr>
        <?php
		}//while
	}else{ ?>
