...
require_once("admin/network/upload_image.php");
	...
	try{
		if( is_array($_POST) && sizeof($_POST) > 0 ){
			if($T->Check()) {
				...
				$validator->addValidation("roll_no","req","Please enter roll no.");
				...
				if($validator->ValidateForm()){
					...
					redirect("network-save.php", $_SERVER['QUERY_STRING']);
	...
	try{	
		if($edit_id > 0){
			$selectFields = '`UID`, `roll_no`, `username`, `email`, `password`, `firstname`, `lastname`, `birthdate`, `gender`, `userimage`, `signupdate`, `lastlogindate`, `is_online`, `is_verified`, `verificationdate`, `verificationcode`, `account_type_id_fk`, `is_active`, `is_blocked`, `blocked_reason`';
  ...
  <div class="panel panel-default">
    <div class="panel-heading">
      <div class="">
        ...
        <div class="fr">
          <input type="button" name="btnback" id="btnback" class="btn btn-primary btn-xs" value=" Back " onClick="javascript:location.href='network-list.php';" />
        </div>
        <div class="clr"></div>
      </div>
    </div>
        ...
        <div>
          <div class=""> <?php echo Message::customMessage(WARNING_UPLOAD_MAX_FILESIZE, 'WARNING'); ?>
		    ...
		    <td width="49%">&nbsp;</td>
              ...
              <tr>
                <td>Roll No</td>
                <td><input class="form-control" name="roll_no" type="text" id="roll_no" value="<?php echo $data['roll_no']?>" size="50" /></td>
              </tr>
              <tr>
                <td><label for="email">Email:&nbsp;<span class="error">*</span></label></td>
                <td><input class="form-control" name="email" type="text" id="email" value="<?php echo $data['email'];?>" size="50" /></td>
              </tr>
              <tr>
              ...
              <tr>
                ...
                <td><input class="form-control" name="firstname" type="text" id="firstname" value="<?php echo $data['firstname'];?>" size="50" /></td>
              </tr>
              <tr>
                <td><label for="lastname">Last Name: <span class="error">*</span></label></td>
                <td><input class="form-control" name="lastname" type="text" id="lastname" value="<?php echo $data['lastname'];?>" size="50" /></td>
              </tr>
              <tr>
                <td><label for="birthdate">Birthdate: <span class="error">*</span></label></td>
                <td><input type="text" name="birthdate" id="birthdate" value="<?php echo $data['birthdate'];?>" readonly="readonly" />
        ...
        ?>
              ...
              <tr id="tr_image_1_preview" style="display:<?php echo $tr_image_1_preview?>">
                <td valign="top"><label for="thumbnail">Thumbnail:</label>
                  <label><small>(png, jpg, jpeg, gif)</small></label></td>
                <td valign="top"><?php
					$imagePath = ABSPATH .'/uploads/user/'.$edit_id.'/small/'.$data['userimage'];
					if(is_file($imagePath)){
						echo '<img src="'.SITEROOT.'/uploads/user/'.$edit_id.'/small/'.$data['userimage'].'" />';
					} else {
						echo '';
					}
				?>
                  &nbsp; <span><a href="javascript:void(0);" onclick="javascript:deleteImage('<?php echo $edit_id; ?>', '<?php echo $data['userimage']; ?>', '1')">Delete</a></span></td>
              </tr>
              <tr>
                <td>&nbsp;</td>
                <td><input type="checkbox" name="is_verified" id="is_verified" value="1" <?php echo ($data['is_verified'] == "1" ? "checked=\"checked\"" : "")?>/>
                  &nbsp;
                  <label for="is_verified">Is verified</label></td>
              </tr>
              <tr>
                <td>&nbsp;</td>
                <td><input type="checkbox" name="is_active" id="is_active" value="1" <?php echo ($data['is_active'] == "1" ? "checked=\"checked\"" : "")?> />
                  &nbsp;
              ...
              <tr>
                ...
                <td><input type="submit" name="btnsubmit" id="btnsubmit" class="btn btn-primary" value=" Save User Information " />
                  &nbsp;
                  <?php
			if($edit_id > 0 && $data['is_verified'] < 1){?>
