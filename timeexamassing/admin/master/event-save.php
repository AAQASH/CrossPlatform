...
include_once('../bootstrap.php');
	...
	try{
		if($edit_id > 0){
			$stmt = $db->query('select * from mts_event where ID=?', $edit_id);
			$data = $stmt->fetch();
			if(count($data) > 0){
				extract($data);
				unset($data);
...
<div id="maincont" class="ovfl-hidden">
  <div class="box">
    <div class="heading">
      <div class="mainhead">
        <h3 class="fl">Save Event </h3>
        <div class="fr">
          <input type="button" name="btnback" id="btnback" value=" Back " onClick="javascript:location.href='event-list.php';" />
        </div>
        <div class="clr"></div>
      </div>
    </div>
    <div class="content"><?php echo Message::getMessage(); ?>
      <form name="save_attribute_info" id="save_event_info" method="post" enctype="multipart/form-data">
        ...
        <div>
          <div class="">
            <table width="100%" class="form">
              ...
              <tr>
                <td>&nbsp;</td>
                <td><input type="submit" name="btnsubmit" id="btnsubmit" value=" Save Event " class="submitbutton" /></td>
                <td>&nbsp;</td>
              </tr>
            </table>
          </div>
