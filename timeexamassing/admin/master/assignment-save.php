...
include_once('../bootstrap.php');
	...
	try{
		if( is_array($_POST) && sizeof($_POST) > 0 ){
			...
			if($validator->ValidateForm()){
				...
				$data["decription"] 	= $input->post('description');
	...
	try{
		if($edit_id > 0){
			$stmt = $db->query('select * from mts_assignment where ID=?', $edit_id);
			$data = $stmt->fetch();
			if(count($data) > 0){
				extract($data);
				unset($data);
...
<div id="maincont" class="ovfl-hidden">
  <div class="box">
    <div class="heading">
      <div class="mainhead">
        <h3 class="fl">Save Assignment </h3>
        <div class="fr">
          <input type="button" name="btnback" id="btnback" value=" Back " onClick="javascript:location.href='assignment-list.php';" />
        </div>
        <div class="clr"></div>
      </div>
    </div>
          ...
          <div class="">
            <table width="100%" class="form">
              ...
              <tr>
                ...
                <td colspan="2">
                <textarea cols="80" id="description" name="description" rows="10"><?php echo $decription?></textarea>
                  <div class="error" id="error_description"></div>
                  <div><small>(No characters limit.)</small></div>                </td>
              </tr>
              <tr id="tr_input_type" style="color:#000000">
                <td width="25%" style="color:#000000"><label for="input_type">Class:<span class="error">*</span></label></td>
              ...
              <tr>
                <td>&nbsp;</td>
                <td><input type="submit" name="btnsubmit" id="btnsubmit" value=" Save Assignment " class="submitbutton" /></td>
                <td>&nbsp;</td>
              </tr>
            </table>
          </div>
