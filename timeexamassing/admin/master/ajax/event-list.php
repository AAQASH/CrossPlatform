...
include_once("../../bootstrap.php");
		...
		case "active":
			try{
				if(is_array($list_ids) && sizeof($list_ids) > 0){
					$data = array();
					$data['is_active'] = 1;
					$db->update('mts_event', $data, "ID in (".$ids.")");
	...
	if(strlen($_REQUEST['ID']) > 0){
		$grid->andCondition("mts_event.ID like '%".$input->post('ID')."%'");
	}
	if(strlen($_REQUEST['attribute_code']) > 0){
		$grid->andCondition("mts_event.name like '%".$input->post('name')."%'");
	}
	if(strlen($_REQUEST['class']) > 0){
		$grid->andCondition("mts_event.description like '%".$input->post('description')."%'");
	}
	if(strlen($_REQUEST['date']) > 0){
		$grid->andCondition("mts_event.date like '%".$input->post('date')."%'");
	}
	if(strlen($_REQUEST['status']) > 0){
		$grid->andCondition("mts_event.status = '".$input->post('status')."'");
	}
	if(strlen($_REQUEST['time']) > 0){
		$grid->andCondition("mts_event.time = '".$input->post('time')."'");
	}
	if(strlen($_REQUEST['is_active']) > 0){
		$grid->andCondition("mts_event.is_active = '".$input->post('is_active')."'");
	}
	
	
	/* SEARCH QUERY */
	try{
		$select = "SQL_CALC_FOUND_ROWS mts_event.*";
		$order_by = (strlen(trim($_REQUEST["txt_column"]." ".$_REQUEST["txt_order_type"])) > 0 ? trim($_REQUEST["txt_column"]." ".$_REQUEST["txt_order_type"]) : "" );
		$grid->select($select)->from('mts_event')
			 ->where("1=1")
			 ->orderby($order_by)
			 ->limit();
		$row = $grid->query();
...
<div class="fr" style="margin-top:15px;">
  ...
  <div class="fr">
	...
	&nbsp;
    <input type="button" name="btnaddnew" id="btnaddnew"  class="btn btn-warning btn-sm" value=" Add New Event " onclick="javascript:location.href='event-save.php';" />
  </div>
  <div class="clr"></div>
</div>
<div class="clr"></div>
	...
	if(sizeof($row) > 0){
        ...
        <tr>
          ...
          <td align="center">
          <a href="<?php echo SITEROOTADMIN?>/master/event-save.php?edit_id=<?php echo $row[$i]["ID"]?>" title="Edit"><span class="glyphicon glyphicon-pencil"></span></a>&nbsp;
