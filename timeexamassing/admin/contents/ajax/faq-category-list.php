...
include_once("../../bootstrap.php");
	...
	$_REQUEST["txt_column"] = (strlen(trim($_REQUEST["txt_column"])) > 0 ? trim($_REQUEST["txt_column"]) : "faq_category_id");
		...
		case "delete":
			try{
				if(is_array($arr_ids) && sizeof($arr_ids) > 0){
					$db->delete('mts_faq_category', "faq_category_id in (".$ids.")");
		...
		case "active":
			try{
				if(is_array($arr_ids) && sizeof($arr_ids) > 0){
					$data = array();
					$data['is_active'] = 1;
					$db->update('mts_faq_category', $data, "faq_category_id in (".$ids.")");
	...
	if(strlen($_REQUEST['faq_category_id']) > 0){
		$grid->andCondition("faq_category_id like '%".$input->post('faq_category_id')."%'");
	}
	...
	try{
		...
		$grid->select($select)->from('mts_faq_category')->where(1)->orderby($order_by)->limit();
		$row = $grid->query();
		$grid->SQL;
...
<div class="fl">
  <h1><small>FAQ Category List</small></h1>
</div>
...
<input type="hidden" size="1" name="txt_column" id="txt_column" value="<?php echo stripcslashes($_REQUEST['txt_column']); ?>"/>
  ...
  <div class="fr">
	...
	&nbsp;
              <input type="button" name="btnaddnew" id="btnaddnew"  class="btn btn-warning btn-sm" value=" Add New FAQ Category " onclick="javascript:location.href='faq-category-save.php';" />
  </div>
  <div class="clr"></div>
</div>
<div class="clr"></div>
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0" id="my_table" class="tblformat">
      <thead>
        <tr>
          ...
          <th width="5%" style="cursor:pointer;" abbr="faq_category_id"><div>ID</div></th>
          <th width="70%" style="cursor:pointer;" abbr="category_name"><div>Category</div></th>
          <th width="15%" style="cursor:pointer;" abbr="is_active"><div>Active/Inactive</div></th>
          <th width="" style="cursor:pointer;" abbr=""><div>Action</div></th>
        </tr>
        <tr class="search">
          <td align="center"><input type="checkbox" name="checkall" id="checkall" value="1" /></td>
          <td><input class="form-control" type="text" name="faq_category_id" id="faq_category_id" value="<?php echo stripcslashes($_REQUEST['faq_category_id']); ?>" size="3" /></td>
	...
	if(sizeof($row) > 0){
		for($i=0; $i < sizeof($row); $i++){
			?>
        <tr>
          <td align="center"><input type="checkbox" name="id[]" id="id[]" value="<?php echo $row[$i]["faq_category_id"]?>" /></td>
          <td><?php echo $row[$i]["faq_category_id"]; ?></td>
          <td><?php echo $row[$i]["category_name"]; ?></td>
          <td><?php echo ($row[$i]["is_active"] == 1 ? "Active" : "Inactive");?></td>
          <td align="center"><a href="<?php echo SITEROOTADMIN?>/contents/faq-category-save.php?edit_id=<?php echo $row[$i]["faq_category_id"]?>" title="Edit"><span class="glyphicon glyphicon-pencil"></span></a>&nbsp;<a href="javascript:void(0);" class="delete" id="<?php echo $row[$i]["faq_category_id"]?>"><span class="glyphicon glyphicon-remove"></span></a></td>
        </tr>
        <?php
		}//while
	}else{ ?>
