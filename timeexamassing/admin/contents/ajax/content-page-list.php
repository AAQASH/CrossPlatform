...
include_once("../../bootstrap.php");
	...
	try{
		...
		$grid->select($select)->from('mts_contentpage')->where("page_type='page'")->orderby($order_by)->limit();
		$row = $grid->query();
		$grid->SQL;
...
<div class="fl">
  <h1><small>Content Page  List</small></h1>
</div>
<div class="fr" style="margin-top:15px;">
  ...
  <div class="fr">
	...
	&nbsp;
	<input type="button" name="btnaddnew" id="btnaddnew" value=" Addnew Content Page " class="btn btn-warning btn-sm" onclick="javascript:location.href='content-page-save.php';" />
  </div>
  <div class="clr"></div>
</div>
<div class="clr"></div>
	...
	if(sizeof($row) > 0){
        ...
        <tr>
          ...
          <td><a href="<?php echo SITEROOTADMIN?>/contents/content-page-save.php?edit_id=<?php echo $row[$i]["contentpage_id"]?>" title="Edit"><span class="glyphicon glyphicon-pencil"></span></a>
