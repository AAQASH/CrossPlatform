...
class Token {
	...
	function tokenDelAll() {
		...
		foreach ($sessvars as $var) { 
			$_x = substr( $var, 0, 7 );
			if( $_x == 'spackt_' ){
				if(@substr_compare($var,"spackt_",0,7)==0) $tokens[]=$var;
			}
		}
	...
	function protectLink($link) {
		if(strpos($link,"?")) return $link."&token=".$this->token;
		else return $link."?token=".$this->token;
	}
	...
	function Check() {
		...
		if(isset($_REQUEST['token'])) {
			...
			if(isset($_SESSION["spackt_".$_REQUEST['token']])) {
				// Check if the token isn't empty
				if(isset($_SESSION["spackt_".$_REQUEST['token']])) {
					$age = time()-$_SESSION["spackt_".$_REQUEST['token']];
