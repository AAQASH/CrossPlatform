...
class input {
	...
	var $global_xss_filtering = TRUE;
	var $enable_query_strings = TRUE;
	var $charset = "UTF-8";
	var $proxy_ips = '';
	...
	*/
	function Input()
	{
		...
		$this->use_xss_clean	= ($this->global_xss_filtering === TRUE) ? TRUE : FALSE;
		$this->allow_get_array	= ($this->enable_query_strings === TRUE) ? TRUE : FALSE;
	...
	*/
	function _clean_input_data($str)
	{
		...
		{
			# changes made by amols 
			# commented call to $this->xss_clean($str)
			# reason: $this->_clean_input_data($str) called in xss_clean so don't need to call xss_clean here
		...
		{
			# changed made by amols
			# original statement $str = str_replace(array("\r\n", "\r"), "\n", $str);
			# replaced with
	...
	*/
	function ip_address()
	{
		...
		if ($this->proxy_ips != '' && $this->server('HTTP_X_FORWARDED_FOR') && $this->server('REMOTE_ADDR'))
		{
	...
	function xss_clean($str, $is_image = FALSE)
	{
		...
		*/
		do
		{
			...
			{
				# commented by amols 
				# comment reason: image tag was not working in ckeditor because of this line
		...
		*/
		...
		$str = $this->_clean_input_data($str);
